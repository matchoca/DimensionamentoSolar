...
function atualizarPotenciaSugerida() {
  ...
  document.getElementById("potenciaSugerida").textContent =
    potenciaSugerida.toFixed(2);
}
...
function atualizaPotenciaReal() {
  ...
  if (
    !selectModulo ||
    !inputQuantidadeModulos ||
    !selectInversor ||
    !inputQuantidadeInversor ||
    !outputPotenciaReal ||
    !outputFDI ||
    !outputArea ||
    !outputPeso
  ...
  const inversorSelecionado = inversoresDB.find(
    (i) => i.id === idInversorSelecionado
  );

  if (
    !inversorSelecionado ||
    isNaN(quantidadeInversores) ||
    quantidadeInversores <= 0
  ) {
  ...
  const fdi =
    potenciaReal / (inversorSelecionado.potenciaNominal * quantidadeInversores);

  const areaTotal =
    moduloSelecionado.moduleHeight *
    moduloSelecionado.moduleWidth *
    quantidadeModulos;
...
function atualizarGrafico() {
  ...
  const inputs = document.querySelectorAll(".consumo");
  const consumoMensal = Array.from(inputs).map(
  ...
  const geracaoMensal = irradianciaMensal.map(
    (mes) => potenciaInstalada * mes.valor * 30 * 0.8
  );
